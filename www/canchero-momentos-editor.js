...
const FILTERS = [
  { n:'Normal', f:'none' },
  { n:'Cancha', f:'saturate(1.4) contrast(1.1)' },
  { n:'B/N', f:'grayscale(1)' },
  { n:'Vintage', f:'sepia(0.5) contrast(1.1) brightness(1.05)' },
  { n:'Frío', f:'hue-rotate(-15deg) saturate(1.2)' },
  { n:'Cálido', f:'sepia(0.25) saturate(1.5) brightness(1.05)' },
  { n:'Drama', f:'contrast(1.4) brightness(0.95) saturate(1.2)' },
];
...
const MOM_CATS = ['Mi Día','Viendo el partido','Jugando','Goles','Entrenamiento','Camisetas','Lifestyle','Moda','Hinchada','Previa'];
const MOM_CAT_LABEL = { 'Mi Día':'Momento' };
const MOM_CAT_BX = { 'Mi Día':'bx-camera','Viendo el partido':'bx-tv','Jugando':'bx-football','Goles':'bx-target-lock','Entrenamiento':'bx-dumbbell','Camisetas':'bx-closet','Lifestyle':'bx-coffee','Moda':'bx-shopping-bag','Hinchada':'bx-flag','Previa':'bx-drink' };
...
function _momShowEditor(){
  ...
  m.innerHTML=`
    ...
    <div style="flex:0 0 auto;display:flex;align-items:center;justify-content:space-between;padding:6px 14px 12px;background:#000;z-index:5;">
      ...
      <div style="display:flex;gap:8px;">
        <button onclick="window._momAddText()" title="Texto" style="background:rgba(255,255,255,0.1);border:none;color:#fff;width:38px;height:38px;border-radius:50%;font-size:18px;cursor:pointer;font-weight:900;">Aa</button>
        <button onclick="window._momToggleStickers()" title="Stickers" style="background:rgba(255,255,255,0.1);border:none;color:#fff;width:38px;height:38px;border-radius:50%;font-size:18px;cursor:pointer;"><i class='bx bx-sticker'></i></button>
        ${isVid?'':`<button onclick="window._momToggleDraw(this)" title="Dibujar" style="background:rgba(255,255,255,0.1);border:none;color:#fff;width:38px;height:38px;border-radius:50%;font-size:18px;cursor:pointer;"><i class='bx bx-pencil'></i></button>`}
      </div>
    </div>
    <div id="mom-canvas-wrap" style="flex:1;position:relative;overflow:hidden;background:#000;display:flex;align-items:center;justify-content:center;">
      ${isVid
        ? `<video id="mom-media" src="${E.mediaURL}" autoplay loop muted playsinline style="max-width:100%;max-height:100%;filter:${E.filter};"></video>`
        : `<img id="mom-media" src="${E.mediaURL}" style="max-width:100%;max-height:100%;filter:${E.filter};">`}
    ...
    <!-- Filtros -->
    <div style="flex:0 0 auto;display:flex;gap:8px;overflow-x:auto;padding:10px 14px;background:#000;scrollbar-width:none;">
      ${FILTERS.map(f=>`<button onclick="window._momSetFilter('${f.f}',this)" data-f="${f.f}" style="flex-shrink:0;background:${f.f===E.filter?'var(--accent)':'#1a1a1a'};color:${f.f===E.filter?'#000':'#aaa'};border:none;border-radius:18px;padding:7px 14px;font-size:12px;font-weight:800;cursor:pointer;">${f.n}</button>`).join('')}
    </div>
    <!-- Stickers (oculto) -->
    ...
    <div style="flex:0 0 auto;padding:8px 14px 4px;background:#000;">
      ...
      <div id="mom-cat-bar" style="display:flex;gap:7px;overflow-x:auto;scrollbar-width:none;padding-bottom:2px;">
        ${MOM_CATS.map(c=>`<button onclick="window._momSetCategory('${c}',this)" data-cat="${c}" style="flex-shrink:0;padding:7px 13px;border-radius:18px;border:2px solid ${c===E.category?'var(--accent)':'#2a2a2a'};background:${c===E.category?'rgba(186,255,0,0.12)':'transparent'};color:${c===E.category?'var(--accent)':'#888'};font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;">${MOM_CAT_ICONS[c]} ${MOM_CAT_LABEL[c]||c}</button>`).join('')}
      </div>
    </div>
    <!-- Publicar -->
    <div style="flex:0 0 auto;padding:8px 14px calc(12px + env(safe-area-inset-bottom));background:#000;display:flex;gap:10px;align-items:center;">
      <input id="mom-caption" placeholder="Escribí algo..." style="flex:1;background:#1a1a1a;border:1px solid #333;border-radius:22px;color:#fff;padding:11px 16px;font-size:14px;outline:none;">
      <button onclick="window._momPublishStory()" id="mom-pub-btn" style="background:var(--accent);color:#000;border:none;border-radius:24px;padding:11px 20px;font-weight:900;font-size:14px;cursor:pointer;white-space:nowrap;">Compartir <i class='bx bx-send'></i></button>
    </div>`;
  E.texts=[]; E.stickers=[];
}
...
window._momPublishStory = async function(){
  const btn=document.getElementById('mom-pub-btn'); if(btn){btn.disabled=true;btn.textContent='Subiendo...';}
