...
window.CancheroSocial = (function(){
  ...
  async function toggleSave(postId, btn){
    ...
    try {
      if (saved){
        await client.from('saved_posts').delete().eq('email',u.email).eq('post_id',postId);
        ...
        await client.from('saved_posts').insert({ email:u.email, post_id:postId });
  ...
  async function toggleSaveMomento(momId, btn){
    ...
    const saved = icon && icon.classList.contains('bxs-bookmark');
    try {
      if (saved){ await client.from('saved_posts').delete().eq('email',u.email).eq('post_id',momId); if(icon) icon.className='bx bx-bookmark'; if(window.showToast) showToast('Quitado de guardados','info'); }
      else { await client.from('saved_posts').insert({ email:u.email, post_id:momId, kind:'momento' }); if(icon) icon.className='bx bxs-bookmark'; if(window.showToast) showToast('Reel guardado','success'); }
    } catch(e){ if(window.showToast) showToast('No se pudo guardar','error'); }
  }
  ...
  async function openSaved(){
    ...
    try {
      const { data: saved } = await client.from('saved_posts').select('post_id,kind').eq('email',u.email).order('created_at',{ascending:false}).limit(150);
      if (!saved || !saved.length){ body.innerHTML='<div style="text-align:center;color:#555;padding:40px;font-size:13px;"><i class="bx bx-bookmark" style="font-size:42px;display:block;margin-bottom:10px;opacity:0.4;"></i>No guardaste nada todavía. Tocá el marcador en cualquier publicación o reel.</div>'; return; }
  ...
  async function refreshSavedMarks(){
    ...
    try {
      const { data: saved } = await client.from('saved_posts').select('post_id').eq('email',u.email).limit(200);
  ...
  async function renderSuggestions(){
    ...
    try {
      ...
      const { data: users } = await client.from('users').select('email,name,photo,role,city').limit(120);
      ...
      const card = x => {
        ...
        const av = x.photo ? `background-image:url('${x.photo}');background-size:cover;background-position:center;` : 'background:var(--accent);display:flex;align-items:center;justify-content:center;color:#000;font-weight:900;font-size:22px;';
