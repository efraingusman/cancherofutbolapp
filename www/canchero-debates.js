...
const SEED = [
  ...
  { title:'¿Quién gana el Mundial 2026?', category:'Mundial', options:['Argentina','Brasil','Francia','España'], local_cover:'img/debates/mundial-2026.jpg', cover_url:COVER_IMG.trophy },
...
window._renderDebatesList = async function(){
  ...
  // Dedupe reales por título (quedarse con el más reciente = primero)
  const seen = {}; const realUnique = [];
  rows.forEach(d=>{ const k=(d.title||'').trim().toLowerCase(); if(!seen[k]){ seen[k]=1; realUnique.push(d); } });
  // Semillas: solo si no existe un debate real con ese título
  const seeds = SEED.filter(s=>!seen[s.title.trim().toLowerCase()]).map((s,i)=>({ id:'seed-'+i, _seed:true, title:s.title, category:s.category, options:s.options, cover_url:(s.local_cover||s.cover_url) }));
  let all = [...realUnique, ...seeds];
...
function _debBody(phase,d,args,opts){
  ...
  return sideHeader + (_debArgsFeed(args,myVotes,false) || '<div style="text-align:center;color:#555;padding:24px;font-size:13px;">Sé el primero en argumentar. Elegí tu bando abajo 👇</div>');
}
