...
const CATS_FEED = ['Mi Día','Viendo el partido','Jugando','Goles','Entrenamiento','Camisetas','Lifestyle','Moda','Hinchada','Previa'];
const CATS_UPLOAD = CATS_FEED;
const CATS_EXPLORER = ['Todos',...CATS_FEED];
const CAT_LABEL = { 'Mi Día':'Momento' };  // display: la clave vieja se muestra como "Momento"

const CAT_BX = {
  'Todos':'bx-grid-alt','Mi Día':'bx-camera','Viendo el partido':'bx-tv','Jugando':'bx-football',
  'Goles':'bx-target-lock','Entrenamiento':'bx-dumbbell','Camisetas':'bx-closet',
  'Lifestyle':'bx-coffee','Moda':'bx-shopping-bag','Hinchada':'bx-flag','Previa':'bx-drink',
...
window._loadMomentosBlock = async function(){
  ...
  const circles = CATS_FEED.map(cat => {
    ...
    if (cat === 'Mi Día') {
      ...
      const avatarCss = (window._avatarBgCss && window._avatarBgCss(u)) || null;
...
window._openMomentosStories = async function(cat){
  ...
  async function render(idx){
    ...
    v.innerHTML = `
      ...
      <div style="position:absolute;top:calc(env(safe-area-inset-top,0px) + 22px);left:10px;right:10px;display:flex;align-items:center;gap:10px;z-index:2;">
        <div onclick="event.stopPropagation();window.openPlayerActions?window.openPlayerActions('${(mo.user_email||'').replace(/'/g,"\\'")}','${(mo.user_name||'').replace(/'/g,"\\'")}'):(window.viewUserProfile&&window.viewUserProfile('${(mo.user_email||'').replace(/'/g,"\\'")}'))" style="width:34px;height:34px;border-radius:50%;${(mo.user_photo||mo.user_avatar)?`background-image:url('${mo.user_photo||mo.user_avatar}');background-size:cover;background-position:center;`:'background:var(--accent);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:#000;'}flex-shrink:0;cursor:pointer;border:1.5px solid rgba(255,255,255,0.5);">${(mo.user_photo||mo.user_avatar)?'':(mo.user_name||'?')[0].toUpperCase()}</div>
      ...
      <!-- Zonas táctiles (izq=prev, der=next) -->
      <div id="mom-tap-prev" style="position:absolute;top:0;left:0;width:35%;height:80%;z-index:3;cursor:pointer;"></div>
      <div id="mom-tap-next" style="position:absolute;top:0;right:0;width:65%;height:80%;z-index:3;cursor:pointer;"></div>`;
  ...
  function goNext(){
    ...
    else { v.remove(); window._loadMomentosBlock(); }
  }
  function goBack(){
    clearTimeout(progressTimer);
...
window._openMomentosExplorer = function(){
  ...
  m.innerHTML = `
    <div style="position:sticky;top:0;z-index:5;background:rgba(10,10,10,0.96);backdrop-filter:blur(10px);border-bottom:1px solid #1a1a1a;padding:calc(10px + env(safe-area-inset-top)) 14px 10px;">
      ...
      <div style="display:flex;gap:7px;overflow-x:auto;scrollbar-width:none;">
        ${CATS_EXPLORER.map(c=>`<button onclick="window._momSetCat('${c}')" id="momcat-${c}" style="flex-shrink:0;padding:6px 13px;border-radius:18px;border:none;font-size:12px;font-weight:700;cursor:pointer;background:${c===window._momCat?'var(--accent)':'#1a1a1a'};color:${c===window._momCat?'#000':'#aaa'};">${CAT_ICONS[c]} ${(typeof CAT_LABEL!=='undefined'&&CAT_LABEL[c])||c}</button>`).join('')}
      </div>
    </div>
...
window._openCrearMomento = function(defaultCat){
  ...
  const catButtons = CATS_UPLOAD.map(c =>
    `<button onclick="window._momSelectCat('${c}',this)" data-cat="${c}" style="flex-shrink:0;padding:8px 14px;border-radius:20px;border:2px solid ${c===selCat?'var(--accent)':'#2a2a2a'};background:${c===selCat?'rgba(186,255,0,0.12)':'transparent'};color:${c===selCat?'var(--accent)':'#777'};font-size:12px;font-weight:700;cursor:pointer;transition:.15s;">${CAT_ICONS[c]} ${(typeof CAT_LABEL!=='undefined'&&CAT_LABEL[c])||c}</button>`
  ).join('');
  ...
  m.innerHTML = `<div style="background:#0f110f;border:1px solid #1e1e1e;border-radius:24px 24px 0 0;width:100%;max-width:480px;padding:20px 16px calc(24px + env(safe-area-inset-bottom));max-height:94vh;overflow-y:auto;box-sizing:border-box;">
    ...
    <div style="margin-bottom:14px;">
      ...
      <div id="mom-cat-buttons" style="display:flex;gap:7px;overflow-x:auto;scrollbar-width:none;padding-bottom:4px;">
        ${catButtons}
      </div>
...
function _openStoriesWithItems(items, cat){
  ...
  function goNext(){ clearTimeout(progressTimer); if(curIdx<items.length-1){curIdx++;render(curIdx);}else{v.remove();} }
