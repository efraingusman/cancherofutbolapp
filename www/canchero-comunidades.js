...
C.post = async function(id){
  ...
  try {
    await s.from('community_posts').insert({ community_id: id, user_email: me().email, user_name: me().name || (me().email||'').split('@')[0], user_photo: me().photo || null, content: txt, votes: 0 });
    inp.value = '';
    C.loadPosts(id);
  } catch(e){ toast('Error: '+(e.message||''),'error'); }
};
...
C.postHtml = function(p){
  const av = p.user_photo ? `background-image:url('${p.user_photo}');background-size:cover;background-position:center;` : 'background:#1a1a1a;';
  return `<div style="display:flex;gap:10px;background:#111;border:1px solid #1e1e1e;border-radius:14px;padding:12px 14px;margin-bottom:10px;">
    ...
    <div style="flex:1;min-width:0;">
      <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
        <div style="width:24px;height:24px;border-radius:50%;${av}flex-shrink:0;"></div>
