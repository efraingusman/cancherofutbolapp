...
function _ftTimeline(){
  ...
  if (cap && !finished) {
    html += `<div style="display:flex;gap:8px;margin-top:16px;">
        <button onclick="window._fAddEventPrompt('goal')" style="flex:1;background:var(--accent);color:#000;border:none;border-radius:10px;padding:10px;font-weight:900;font-size:12px;cursor:pointer;">⚽ Gol</button>
        <button onclick="window._fAddEventPrompt('assist')" style="flex:1;background:rgba(186,255,0,0.1);color:var(--accent);border:1px solid rgba(186,255,0,0.35);border-radius:10px;padding:10px;font-weight:800;font-size:12px;cursor:pointer;">🅰️ Asistencia</button>
        <button onclick="window._fAddSubstitution()" style="flex:1;background:rgba(100,180,255,0.1);color:#64b4ff;border:1px solid rgba(100,180,255,0.35);border-radius:10px;padding:10px;font-weight:800;font-size:12px;cursor:pointer;"><i class='bx bx-transfer'></i> Cambio</button>
      </div>
...
window._fAddEventPrompt = function(type){
  ...
  const col=(arr,name,team)=>`<div style="flex:1;min-width:0;"><div style="font-size:11px;font-weight:800;color:#aaa;margin-bottom:8px;">${name}</div>${arr.length?arr.map(p=>`<button onclick="window._fSaveEvent('${type}','${(p.player_email||'').replace(/'/g,"")}','${(p.player_name||'Jugador').replace(/'/g,"")}','${team}')" style="display:flex;align-items:center;gap:8px;width:100%;text-align:left;background:#141414;border:1px solid #222;border-radius:10px;padding:9px;margin-bottom:6px;cursor:pointer;color:#fff;">${p.player_photo?`<img src="${window.avatarUrl?window.avatarUrl(p.player_photo,28):p.player_photo}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;object-position:center 28%;">`:`<div style="width:28px;height:28px;border-radius:50%;background:#1a2a1a;display:flex;align-items:center;justify-content:center;color:var(--accent);font-weight:900;font-size:12px;">${(p.player_name||'?')[0]}</div>`}<span style="font-size:12px;font-weight:600;">${p.player_name||'Jugador'}</span></button>`).join(''):'<div style="color:#555;font-size:11px;">Sin jugadores</div>'}</div>`;
  ov.innerHTML=`<div style="background:#0f110f;border:1px solid #1f2a1f;border-radius:18px;width:100%;max-width:560px;padding:18px;max-height:80vh;overflow-y:auto;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;"><div style="font-size:14px;font-weight:900;">${type==='goal'?'⚽ ¿Quién hizo el gol?':'🅰️ ¿Quién dio la asistencia?'}</div><button onclick="document.getElementById('f-ev-picker').remove()" style="background:rgba(255,255,255,0.06);border:1px solid #333;color:#fff;width:30px;height:30px;border-radius:50%;cursor:pointer;">&times;</button></div>
