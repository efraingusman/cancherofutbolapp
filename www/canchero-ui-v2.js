...
window._rankLoad = async function(){
  ...
  try {
    if (st.mode === 'clubes') {
      const res = await client.from('clubs').select('id,name,logo,logo_url,city,country,stats').limit(300);
      rows = ((res&&res.data)||[]).map(c => {
        const s = c.stats||{};
        return { name:c.name, img:c.logo_url||c.logo, city:c.city, country:c.country, val:_calcVal(s,cat), isClub:true };
      });
    } else {
      // INCLUYE A TODOS: no exigimos stats
      const res = await client.from('users').select('email,name,photo,avatar_url,city,country,stats').limit(500);
      rows = ((res&&res.data)||[]).map(u => {
        const s = u.stats||{};
        return { name:u.name||'Anónimo', img:u.avatar_url||u.photo, city:u.city, country:u.country, email:u.email, val:_calcVal(s,cat) };
      });
    }
  ...
  list.innerHTML = rest.length ? rest.map((r,i)=>{
    ...
    return `<div style="display:flex;align-items:center;gap:12px;padding:11px 12px;border-bottom:1px solid #1a1a1a;${me?'background:rgba(186,255,0,0.08);':''}">
      ...
      <div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:14px;color:${me?'var(--accent)':'#fff'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${r.name}</div><div style="font-size:11px;color:#666;">${loc}</div></div>
